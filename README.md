...
見出しはH1からH5まで対応しており、自動採番が付く。段落、箇条書き、番号付きリストはネストにも対応する。表には自動で表番号が振られ、画像には図番号が付く。採番形式は設定で連番・章番号付きを切り替えられる。コードブロックと水平線にも対応している。インライン要素としてはテキスト、コード、太字、斜体、リンクを扱える。
